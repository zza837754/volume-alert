...
# 不同币价格差异大，所以每个品种分开设置
VOLUME_THRESHOLDS = {
    "BTC-USDT-SWAP": 2500,      # 5分钟内成交达到 5500 个 BTC 才推送
    "ETH-USDT-SWAP": 70000,    # 5分钟内成交达到 100000 个 ETH 才推送
}
...
def check_news():
    ...
    # 用链接(url)作为每条新闻的唯一标识
    current_ids = {link for _, link in all_items}   # 记录全部抓到的，不止新鲜的，避免旧新闻一直反复触发判断
